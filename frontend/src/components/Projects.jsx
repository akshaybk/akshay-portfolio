      ...
      <div className="project-content">
        <div className="project-header">
          ...
          {(project.github_url || project.live_url) && (
            <div className="project-links" aria-label={`${title} links`}>
              {project.github_url && <a href={project.github_url} target="_blank" rel="noopener noreferrer" aria-label={`${title} GitHub repository`}>GitHub</a>}
              {project.live_url && <a href={project.live_url} target="_blank" rel="noopener noreferrer" aria-label={`${title} live demo`}>Live</a>}
            </div>
          )}
        </div>
        ...
        {technologies.length > 0 && (
          <div className="technologies" aria-label="Technologies used">
            {technologies.map((technology, index) => <span key={`${technology}-${index}`}>{technology}</span>)}
          </div>
        )}
      </div>
    </article>
